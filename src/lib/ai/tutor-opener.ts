...
export function buildTutorOpener(state: UserState, now = new Date()): TutorOpener | null {
  const mistakes = openMistakes(state);

  // 1. Test imminent and things still unresolved — nothing else matters today.
  if (isCramWindow(state, now) && mistakes.length > 0) {
    const days = daysUntilTest(state, now) ?? 0;
    const when = days === 0 ? "today" : days === 1 ? "tomorrow" : `in ${days} days`;
    const worst = QUESTIONS_BY_ID[mistakes[0].questionId];
    return {
  ...
  if (stubborn) {
    const q = QUESTIONS_BY_ID[stubborn.questionId];
