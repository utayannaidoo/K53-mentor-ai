 ...
 * is the answer), but "Road sign for this question about signs" — what we
 * shipped before — describes nothing, so a blind learner cannot answer a single
 * sign question. Signs are the largest section of the real test, 28 of 64 marks.
 *
 * South African signs follow a strict visual grammar by family, so naming the
 * shape and colour conveys genuine perceptual information while leaving
 * hundreds of candidate signs open. That is the same information a sighted
 * learner gets from a glance.
 *
 * This is a floor, not a ceiling: per-sign visual descriptions ("black symbol
 * of two children walking") belong in the sign catalogue and would let a blind
 * learner answer as precisely as a sighted one. That is a content task.
 */
const FAMILY_DESCRIPTION: Record<string, string> = {
  regulatory: "a regulatory sign — round or octagonal, in red, blue, or black on white",
