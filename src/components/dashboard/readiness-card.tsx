...
export function ReadinessCard({
  ...
  delta: number | null;
  /** Section sitting under its own pass mark, if one is. */
  ...
  // Green for a number that means "you'd fail" reads as a bug. Tone it honestly.
  const passTone =
  ...
  return (
    <Card className={cn(glassFloat, "flex flex-col items-center justify-center p-6 text-center")}>
      ...
      {/* Two numbers far apart look like a bug unless you say why. The real test
          is passed section by section, so one weak section caps the whole
          prediction however much the learner knows overall. */}
      {blocking && (
        <p className="mt-3 max-w-xs text-xs leading-relaxed text-muted-foreground">
          Readiness is how much you know. Predicted pass is stricter, because the
          real test needs the pass mark in <strong>every</strong> section — and{" "}
