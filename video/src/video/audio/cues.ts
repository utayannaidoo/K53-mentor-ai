...
export const VO_SLOTS = [
  ...
  { id: "vo-02", at: 370, maxFrames: 134, line: "So why do six in ten fail?", note: "Moved earlier and widened from 120f: the Edge take runs 2.26s incl. trailing silence, so it now starts with the hook line and still clears slam 1 at 504." },
  ...
  { id: "vo-08", at: 3104, maxFrames: 156, line: "K53 Mentor AI.", note: "Moved onto the mark-draw (3099) and widened from 140f for the Edge take; ends 8f before vo-09. The only time the name is spoken, on the only frame the logo appears." },
  { id: "vo-09", at: 3268, maxFrames: 200, line: "Pass first time.", note: "Last words in the film." },
] as const;

/**
 * The active read. Files live in `public/audio/vo/<VO_VOICE>/vo-01.mp3` …
 *
 * `za-duo` is generated by `scripts/generate-vo-edge.mjs` — Microsoft Edge
 * neural TTS, free, no key. Two South African voices split the film the way
 * its acts do: Luke (en-ZA-LukeNeural) carries the problem, Leah
 * (en-ZA-LeahNeural) carries the product. One folder per read, so an
 * ElevenLabs single-voice take (`generate-vo.mjs`, needs a key) can sit
 * alongside it and switching is this one word.
 *
 * Regenerate with:  node scripts/generate-vo-edge.mjs
 */
export const VO_VOICE = "za-duo";

/**
 * ON. The film was cut to read silent, and every line is also on screen — the
 * narration is a second telling, not a crutch. The two problem lines are
 * delivered by a different voice than the product lines on purpose: the villain
 * act and the hero act do not share a narrator.
 */
