 ...
 */
/**
 * The bed yields to the read. Each VO slot dips the score to ~72% across its
 * window — in over 12 frames, back out over the last 24 — so the narration
 * sits *under* the music as the brief directs, instead of fighting it for the
 * same band. Windows rather than measured audio ends: every take is shorter
 * than its slot, so a window that runs long only means the bed comes back up
 * while the voice is already done.
 */
const voDuck = (f: number): number => {
  let m = 1;
  for (const s of VO_SLOTS) {
    const start = s.at - 12;
    const end = s.at + s.maxFrames;
    if (f < start || f > end) continue;
    const down = interpolate(f, [start, s.at], [1, 0.72], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });
    const up = interpolate(f, [end - 24, end], [0.72, 1], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });
    m = Math.min(m, down, up);
  }
  return m;
};
...
export const SoundDesign: React.FC = () => {
  return (
    <>
      ...
      {/* Voice-over slots. The active read is `za-duo` — see VO_VOICE in
          cues.ts. .mp3 because that is what Edge TTS returns from
          `generate-vo-edge.mjs`; if you record a real session instead, export
          mp3 or change this. */}
      {VO_ENABLED
        ? VO_SLOTS.map((slot) => (
            <Sequence
