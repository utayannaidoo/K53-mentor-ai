...
export const LogoMark: React.FC<{
  ...
  return (
    ...
    >
      ...
      >
        {/* Road edges, drawn from the vanishing point at the top downward —
            two paths so each normalises to its own length and draws in sync.
            Weight is up from the app's 2 to 2.5 — a hairline that reads at
            36px disappears at 300px against a bright fill. */}
        {[
          { d: "M6.5 20.75 10.4 5.2" },
          { d: "M17.5 20.75 13.6 5.2" },
        ].map(({ d }) => (
          <path
            key={d}
            d={d}
        ...
        {/* Lane dashes, tapered with distance like the app mark, and lit
            bottom to top — the same order as `<LaneRush>`. */}
        {[
          { d: "M12 16.75v3", w: 3.0, i: 0 },
          { d: "M12 11.25v2", w: 2.2, i: 1 },
          { d: "M12 6.9v1.2", w: 1.5, i: 2 },
        ].map(({ d, w, i }) => (
          <path
            key={d}
            d={d}
