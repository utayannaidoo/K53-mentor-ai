...
**Voice-over** — seven lines, two South African neural voices (`za-duo` read:
Luke for the problem act, Leah for the product), generated free with Edge TTS —
no key needed. Regenerate with `node scripts/generate-vo-edge.mjs`. Script and
direction: [`docs/VOICEOVER.md`](docs/VOICEOVER.md). The bed ducks under every
VO window; an ElevenLabs path remains in `scripts/generate-vo.mjs` if a key
ever exists.
