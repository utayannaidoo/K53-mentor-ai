...
function buildQueue(state: UserState, limit: number): Question[] {
...
describe("mistakes lead the practice queue", () => {
  it("a missed question comes back in the very next session", () => {
    ...
    // the back of the bank — the one item you've proven you don't know.
    ...
    const queue = buildQueue(state, 12);
  ...
  it("leads with at most MISTAKES_PER_SESSION, so sessions still teach new material", () => {
    ...
    const leading = queue.filter((q) => missedIds.has(q.id));
    expect(leading.length).toBe(MISTAKES_PER_SESSION);
  ...
  it("never fills a session entirely with mistakes, even at a tiny cap", () => {
    ...
    const queue = buildQueue(state, 2);
  ...
  it("does not ask about the same road sign twice in one session", () => {
    ...
    const queue = buildQueue(state, 12);
    const subjects = queue.map(subjectOf);
  ...
  it("a retired mistake stops leading", () => {
    ...
    expect(buildQueue(state, 12)[0].id).not.toBe(missed.id);
  });
});
