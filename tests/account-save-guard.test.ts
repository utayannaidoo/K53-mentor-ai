 ...
 * — the write that turned a Car subscriber's profile into a motorcycle one
 * permanently. The guard must refuse those writes.
 */
...
describe("saveAccount — refuses to write another account's data", () => {
  ...
  it("clamps the written licence code to the paid track", async () => {
    // A Car subscriber whose cached code is a motorcycle one must not push
    // that code back to the server — the row self-heals instead.
    const { client, writes } = fakeSupabase("user-B");
    await saveAccount(
      client,
      stateFor("user-B", "A", { tier: "premium_plus", vehicleClass: "car" }),
    );
    expect(writes.find((w) => w.table === "profiles")?.row.vehicle_code).toBe("8");
  });

  it("leaves a free learner's chosen code alone", async () => {
    const { client, writes } = fakeSupabase("user-B");
    await saveAccount(client, stateFor("user-B", "A", { tier: "free", vehicleClass: null }));
    expect(writes.find((w) => w.table === "profiles")?.row.vehicle_code).toBe("A");
  });
});
